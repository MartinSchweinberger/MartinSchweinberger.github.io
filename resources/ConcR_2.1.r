...
ConcR <- function(path, pattern, context, all.pre = FALSE, exact = FALSE) {
  ...
  conc <- sapply(corpus.files, function(x) {
    txt <- scan(x, what = "char", sep = "", quiet = T, skipNul = T, quote = "")
      file <- as.vector(unlist(gsub("/", "qwertz", x, fixed = T)))
      file <- as.vector(unlist(gsub(".*qwertz", "", file)))
      txt <- paste(txt, collapse = " ")
      txt <- str_replace_all(txt, " {2,}" , " ")
      txt <- str_trim(txt, side = "both")
      lngth <- as.vector(unlist(nchar(txt)))
      ifelse(exact == FALSE, pattern <- paste(" {0,1}[A-Z]{0,1}[a-z]{0,}", pattern, "[a-z]{0,} {0,1}", sep=""), pattern <- paste(" ", pattern, "[ |!|.|?|:|;]{1,}", sep=""))
  ...
  ifelse(ncol(df) == 0, cat("Number of columns does not match the number of column names!\n"),
    ...
    ifelse(ncol(df) > 5, cat("Number of columns does not match the number of column names!")))))
...
#test <- ConcR(path = path, pattern = "[A|a]ll", context = 30, exact = FALSE, all.pre = T)
# inspect results
#head(test)
