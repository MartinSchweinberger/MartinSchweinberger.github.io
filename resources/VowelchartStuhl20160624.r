...
#################################################################
#x <- "C:\\03-MyProjects\\01VowelAnalysis/T0004S.txt"        # for testing
...
# use only items used by b. stuhl
  selection <- c("had", "head", "heed", "hid", "hud")
...
# set up plot
  ...
  symbols(x, y, circles = v3$F1sd, inches = 1/3, bg = addTrans("lightgrey", 100),
    ...
    ylab = "F2 - F1 (Hz)", add = F, main = "Individual Vowel Formant Values (Non-Normalized)")
  # add ipa symbols
  ipa <- c("\u00E6",    # had
    "e",                # head
    "i",                # heed
...
#      ref <- ref[ref$sub == "M", ], ref <- ref)))
  ...
  # add sd to data frame
  ref3 <- data.frame(ref$file[1:5], ref$item[1:5], ref$setting[1:5], F1, F2, F3, F1sd, F2sd, F3sd)
  # add column names
