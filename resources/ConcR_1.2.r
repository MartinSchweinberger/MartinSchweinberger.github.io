...
### --- Schweinberger, Martin. 2015. ConcR - a concordance function for R.
...
ConcR <- function(path, pattern, context, all.pre = FALSE) {
  ...
  conc <- sapply(corpus.files, function(x) {
    txt <- scan(x, what = "char", sep = "", quiet = T, skipNul = T, quote = "")
      ...
      file <- str_replace_all(file, "\\.txt|\\.TXT", "")
  ...
  ifelse(ncol(df) == 4, colnames(df) <- c("file", "pre", "token", "post"), colnames(df) <- c("file", "pre", "token", "post", "all.pre"))
...
###############################################################
# Example
#path <- "C:\\PhD\\skripts n data\\corpora\\ICE Ireland version 1.2.2\\ICE-Ireland txt\\ICE spoken running txt"
#test <- ConcR(path = path, pattern = "[L|l]ike", context = 30, all.pre = FALSE)
# inspect results
#head(test)
