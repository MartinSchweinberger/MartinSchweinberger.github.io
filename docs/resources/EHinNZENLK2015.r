##################################################################
### --- R script "Why We Need Better Statistics -- 
### --- A Case Study on \textit{eh} in New Zealand English"
...
### --- write an email to martin.schweinberger@uni-hamburg.de
### --- CITATION
### --- Schweinberger, Martin. 2015. "Why We Need Better Statistics -- 
### --- A Case Study on \textit{eh} in New Zealand English". Unpublished R script  
### --- (University of Hamburg): www.martinschweinberger.de.
### --- BibTeX
### --- @misc{schweinberger2015betterstatz,
### ---   author = {Schweinberger, Martin},
### ---   title = {Why We Need Better Statistics -- A Case Study on \textit{eh} in New Zealand English},
### ---   year = {2015},
### ---   howpublished = {Unpublished Manuscript (University of Hamburg)},
### ---   note = {www.martinschweinberger.de}  }
###############################################################
###############################################################
###                   START
###############################################################
...
source("C:\\R/PseudoR2lmerBinomial_ms.R")
...
imageDirectory<-"C:\\03-MyProjects\\EHinNZE_NLK\\Beamer\\images"
...
################################################################
# sex age eth priming
mean.eh.agesexethpriming <- by(mydata$suf.eh, list(mydata$priming, mydata$ethnicity, mydata$sex, mydata$age), mean)
sum.eh.agesexethpriming <- by(mydata$suf.eh, list(mydata$priming, mydata$ethnicity, mydata$sex, mydata$age), sum)
sum.su.agesexethpriming <- by(rep(1, nrow(mydata)), list(mydata$priming, mydata$ethnicity, mydata$sex, mydata$age), sum)
# inspect results
...
# create a table with the results
priming <- rep(c("noprime", "prime"), 8)
eth <- rep(c("Maori", "Maori", "Pakeha", "Pakeha"), 4)
sex <- rep(c(rep("Female", 4), rep("Male", 4)), 2)
age <- c(rep("Young", 8), rep("Old", 8))
eh <- sum.eh.agesexethpriming[1:length(mean.eh.agesexethpriming)]
su <- sum.su.agesexethpriming[1:length(mean.eh.agesexethpriming)]-eh
eh.mean <- round(mean.eh.agesexethpriming[1:length(mean.eh.agesexethpriming)], 3)
eh.df2 <- data.frame(age, sex, eth, priming, eh, su, eh.mean)
eh.df2 <- rbind(eh.df2, c("Total", "", "", "", sum(na.omit(eh)), sum(na.omit(su)), round(mean(mydata$suf.eh), 3)))
colnames(eh.df2) <- c("Age", "Sex", "Ethnicity", "Priming", "EH", "Speech Units without EH", "Mean EH (ptw)")
# inspect results
eh.df2

# SAVE DATA
#write.table(eh.df0, "C:\\03-MyProjects\\EHinNZE_NLK\\Beamer/ehdf0.txt", sep="\t")
#write.table(eh.df1, "C:\\03-MyProjects\\EHinNZE_NLK\\Beamer/ehdf1.txt", sep="\t")
#write.table(eh.df3, "C:\\03-MyProjects\\EHinNZE_NLK\\Beamer/ehdf2.txt", sep="\t")
################################################################
################################################################
################################################################
################################################################
...
z <- c(3, z)
# activate (remove #) to save
png("C:\\03-MyProjects\\EHinNZE_NLK\\Beamer\\images/Priming.png") # save plot
...
# activate (remove #) to save
png("C:\\03-MyProjects\\EHinNZE_NLK\\Beamer\\images/EHText.png") # save plot
# make plot
...
# prepare data for visualization
mydatan <- data.frame(mydata$ethnicity, mydata$age, mydata$sex, mydata$suf.eh)
...
################################################################
# age
bivarage <- chisq.test(table(mydata$age, mydata$suf.eh))
bivarage 

# sex
bivarsex <- chisq.test(table(mydata$sex, mydata$suf.eh))
bivarsex

# ethnicity
bivarethnicity <- chisq.test(table(mydata$ethnicity, mydata$suf.eh))
bivarethnicity

# priming
bivarpriming <- chisq.test(table(mydata$priming, mydata$suf.eh))
bivarpriming
...
mydata <- data.frame(mydata$file.speaker.id, mydata$ethnicity, mydata$age, mydata$sex, mydata$priming, mydata$suf.eh)
colnames(mydata) <- gsub("mydata.", "", colnames(mydata))
mydata$file.speaker.id <- as.factor(mydata$file.speaker.id)
...
###########################################################################
###########################################################################
### MODEL FITTING: STEP-WISE STEP-UP
###########################################################################
###########################################################################
###########################################################################
# create models
m0.glm = glm(suf.eh ~ 1, family = binomial, data = mydata) # baseline model glm
m1.glm = glm(suf.eh ~ age, family = binomial, data = mydata)
m2.glm = glm(suf.eh ~ age + sex, family = binomial, data = mydata)
m3.glm = glm(suf.eh ~ age + sex + ethnicity, family = binomial, data = mydata)
m4.glm = glm(suf.eh ~ age + sex + ethnicity + priming, family = binomial, data = mydata)
m5.glm = glm(suf.eh ~ age + sex + ethnicity + priming + age : sex, family = binomial, data = mydata)
m6.glm = glm(suf.eh ~ age + sex + ethnicity + priming + age : sex + sex : ethnicity, family = binomial, data = mydata)
m7.glm = glm(suf.eh ~ age + sex + ethnicity + priming + age : sex + sex : ethnicity + age : ethnicity, family = binomial, data = mydata)
m8.glm = glm(suf.eh ~ age + sex + ethnicity + priming + age : sex + sex : ethnicity + age : ethnicity + age : sex : ethnicity, family = binomial, data = mydata)
# compare models
m0.m1 <- anova(m0.glm, m1.glm, test = "Chi")
age.effect.blr <- anova(m0.glm, m1.glm, test = "Chi") # effect of age
m1.m2 <- anova(m1.glm, m2.glm, test = "Chi")
sex.effect.blr <- anova(m1.glm, m2.glm, test = "Chi") # effect of sex
m2.m3 <- anova(m2.glm, m3.glm, test = "Chi")
ethnicity.effect.blr <- anova(m2.glm, m3.glm, test = "Chi") # effect of ethnicity
m3.m4 <- anova(m3.glm, m4.glm, test = "Chi")
priming.effect.blr <- anova(m3.glm, m4.glm, test = "Chi") # effect of priming
m4.m5 <- anova(m4.glm, m5.glm, test = "Chi")
m5.m6 <- anova(m5.glm, m6.glm, test = "Chi")
m6.m7 <- anova(m6.glm, m7.glm, test = "Chi")
m7.m8 <- anova(m7.glm, m8.glm, test = "Chi")
# model comparision summary
mdlcmps <- list(m0.m1, m1.m2, m2.m3, m3.m4, m4.m5, m5.m6, m6.m7, m7.m8)
# load function
source("C:\\R/ModelFittingSummarySWSULogReg.R") # for Fixed Effects Model fitting: Binary Logistic Models
# apply function
mdl.cmp.blr.swsu <- mdl.fttng.swsu.blr(mdlcmps)
# inspect model comparison summary
mdl.cmp.blr.swsu

# remove # to activate
#write.table(mdl.cmp.blr.swsu, "C:\\03-MyProjects\\EHinNZE\\Article/mdl_cmp_blr_swsu.txt", sep="\t")
# the step-wise step-up model fitting procedure suggests that model 4 with age, 
# sex, ethnicity, and priming is the best model
###########################################################################
### --- MODEL FITTING: STEP-WISE STEP-DOWN
###########################################################################
# compare models
m8.m7 <- anova(m8.glm, m7.glm, test = "Chi")
...
# model comparision summary
mdlcmps <- list(m8.m7, m7.m6, m6.m5, m5.m4, m4.m3, m3.m2, m2.m1, m1.m0)
# load function
source("C:\\R/ModelFittingSummarySWSDLogReg.R") # for Fixed Effects Model fitting: Binary Logistic Models
# apply function
mdl.cmp.blr.swsd <- mdl.fttng.swsd.blr(mdlcmps)
# inspect model comparison summary
mdl.cmp.blr.swsd

# remove # to activate
#write.table(mdl.cmp.blr.swsd, "C:\\03-MyProjects\\EHinNZE\\Article/mdl_cmp_blr_swsd.txt", sep="\t")
# step-wise step-down confirm that the model with all main effects and no interactions, i.e. m4.glm 
# is the FMAM (Final Minimal Adequate Model)
###########################################################################
# Final Minimal Adequat Model
###
m4.lrm <- lrm(suf.eh ~ age + sex + ethnicity + priming, data = mydata, x = T, y = T, linear.predictors = T)
m4.lrm
...
# re-selected with the same size but with placing back the drawn data point
validate(m4.lrm, bw = T, B = 200)

# the validate function shows that retaining 4 predictors is the best option
###########################################################################
# determine penalty
pentrace(m4.lrm, seq(0, 0.8, by = 0.05))
...
lr.glm.spk <- glm(cbind(eh, no.eh) ~ age + sex + ethnicity, data = mydata.by.spk, family = binomial)
...
# 98.73% accuracy! WOW -but wait a second... ;)
###########################################################################
...
# 98.73% accuracy - hm... why that? because both models always preditc no.eh(!)
...
sjp.glm(lr.glm, axisLabels.y = c("age(old)", "sex:male", "ethnicity:maori", "priming:yes", "age(old):sex(male)", "age(old):priming(yes)"), gridBreaksAt = 0.5)
#imageFile <- paste(imageDirectory,"OddsRatiosBinLogReg.png",sep="/")
#ggsave(file = imageFile)
...
# fixed effects binary logistic regression summary
blr <- blrm.summary(lr.glm, lr.lrm, predict.acc)
# activate (remove #) to save
#write.table(blr, "C:\\03-MyProjects\\EHinNZE\\Article/blr.txt", sep="\t")
###########################################################################
###########################################################################
###########################################################################
...
m4.glmer <- glmer(suf.eh ~ age + sex + ethnicity + priming + (1|file.speaker.id), family = binomial, data = mydata, control = glmerControl(optimizer="bobyqa"))
m5.glmer <- glmer(suf.eh ~ age + sex + ethnicity + priming + age : sex + (1|file.speaker.id), family = binomial, data = mydata, control = glmerControl(optimizer = "bobyqa"))
m6.glmer <- glmer(suf.eh ~ age + sex + ethnicity + priming + age : sex + age : ethnicity + (1|file.speaker.id), family = binomial, data = mydata, control = glmerControl(optimizer = "bobyqa"))
m7.glmer <- glmer(suf.eh ~ age + sex + ethnicity + priming + age : sex + age : ethnicity + sex : ethnicity + (1|file.speaker.id), family = binomial, data = mydata, control = glmerControl(optimizer = "bobyqa")) # BEST MODEL
m8.glmer <- glmer(suf.eh ~ age + sex + ethnicity + priming + age : sex + age : ethnicity + sex : ethnicity + age : sex : ethnicity + (1|file.speaker.id), family = binomial, data = mydata, control = glmerControl(optimizer = "bobyqa"))
...
anova(m0.glmer, m1.glmer, m2.glmer, m3.glmer, m4.glmer, m5.glmer, m6.glmer, m7.glmer, m8.glmer, test = "Chi")
...
mdlcmp <- list(m1.m0, m2.m1, m3.m2, m4.m3, m5.m4, m6.m5, m7.m6, m8.m7)
# apply function
mdl.cmp.glmer.swsu <- mdl.fttng.swsu(mdlcmp)
# inspect output
mdl.cmp.glmer.swsu

write.table(mdl.cmp.glmer.swsu, "C:\\03-MyProjects\\EHinNZE\\Article/mdl_cmp_glmer_swsu.txt", sep="\t")
...
mdlcmp <- list(m8.m7, m7.m6, m6.m5, m5.m4, m4.m3, m3.m2, m2.m1, m1.m0)
# apply function
mdl.cmp.glmer.swsd <- mdl.fttng.swsd(mdlcmp)
# inspect results
mdl.cmp.glmer.swsd

# remove # to activate
#write.table(mdl.cmp.glmer.swsd, "C:\\03-MyProjects\\EHinNZE\\Article/mdl_cmp_glmer_swsd.txt", sep="\t")
...
age.effect.glmer <- anova(m0.glmer, m1.glmer, test = "Chi") # effect of age

# we now test the effect of sex by adding sex as a fixed effect
sex.effect.glmer <- anova(m1.glmer, m2.glmer, test = "Chi") # effect of sex
...
ethnicity.effect.glmer <- anova(m2.glmer, m3.glmer, test = "Chi") # effect of ethnicity
...
mlr.lrm <- lrm(suf.eh ~ age + sex + ethnicity + priming, data = mydata, x = T, y = T)
# we now create a lmer object equivalent to the final minimal adequate model
mlr.lmer <- lmer(suf.eh ~ age + sex + ethnicity + priming + (1|file.speaker.id), data = mydata, family = binomial)
...
meblrm.summary(m0.glm, m1.glm, m0.glmer, mlr.glmer, mydata$suf.eh) #  

# mixed effects binary logistic regression summary
meblrm <- meblrm.summary(m0.glm, m1.glm, m0.glmer, mlr.glmer, mydata$suf.eh)  
# activate (remove #) to save
#write.table(meblrm, "C:\\03-MyProjects\\EHinNZE\\Article/meblrm.txt", sep="\t")
